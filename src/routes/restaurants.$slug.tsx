    ...
    return (
      <div className="min-h-screen">
        ...
        <div className="max-w-lg mx-auto px-4 py-20 text-center">
          ...
          <h1 className="text-2xl font-bold mt-4">Reservation confirmed</h1>
          <p className="text-stone-600 mt-2">
            A WhatsApp confirmation was sent to {confirmation.reservation.guestPhone}.
          </p>
          <div className="mt-6 rounded-xl border border-stone-200 bg-white p-6 text-left space-y-2">
            <p><span className="text-stone-500">Restaurant:</span> {confirmation.restaurant.name}</p>
            <p><span className="text-stone-500">Date:</span> {confirmation.reservation.date}</p>
            <p><span className="text-stone-500">Time:</span> {confirmation.reservation.time.slice(0, 5)}</p>
            <p><span className="text-stone-500">Party size:</span> {confirmation.reservation.partySize}</p>
            <p><span className="text-stone-500">Confirmation code:</span> <span className="font-mono font-semibold">{confirmation.reservation.confirmationCode}</span></p>
          </div>
          <a href="/" className="inline-block mt-8 text-amber-700 hover:underline">Book another restaurant</a>
        </div>
      </div>
    )
  }
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 relative">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6 pb-16">
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white rounded-lg border border-stone-200 p-6 shadow-sm">
              ...
              <div className="space-y-5">
                {menu.map((cat) => (
                  <div key={cat.id}>
                    <h3 className="text-sm font-semibold text-amber-700 uppercase tracking-wide">{cat.name}</h3>
          ...
          <div className="bg-white rounded-lg border border-stone-200 p-6 h-fit sticky top-20 shadow-xl">
            <h2 className="font-semibold text-stone-900 mb-4 flex items-center gap-2"><CalendarDays className="h-4 w-4" /> Book a table</h2>
            <div className="space-y-3">
              ...
              <div>
                <label className="text-xs text-stone-500">Area (optional)</label>
                <select
                  value={areaId ?? ''}
              ...
              {selectedTime && (
                <form onSubmit={submitBooking} className="pt-4 border-t border-stone-100 space-y-3 mt-2">
                  ...
                  <div>
                    <label className="text-xs text-stone-500">WhatsApp phone number</label>
                    <input
                      required
                      value={guestPhone}
                  ...
                  <div>
                    <label className="text-xs text-stone-500">Special requests (optional)</label>
                    <textarea
                  ...
                  >
                    {submitting ? 'Booking...' : `Confirm table for ${date} at ${selectedTime}`}
                  </button>
                </form>
              )}
            </div>
