  ...
  return (
    <div className="min-h-screen">
        ...
        <div className="overflow-hidden rounded-lg bg-stone-950 text-white">
          <div className="grid min-h-[320px] lg:grid-cols-[1.1fr_.9fr]">
            <div className="p-7 sm:p-10 flex flex-col justify-center">
              <p className="text-sm font-medium text-amber-300">Live restaurant reservations</p>
              <h1 className="mt-3 text-4xl sm:text-5xl font-bold tracking-tight">
                Find your table tonight
              </h1>
              <p className="text-stone-300 mt-4 max-w-xl">
                Browse local restaurants, check real-time availability, and book in a couple of taps with WhatsApp confirmations.
              </p>
              <div className="mt-6 grid gap-3 rounded-lg bg-white p-2 text-stone-900 shadow-xl sm:grid-cols-[1fr_auto_auto]">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full rounded-md border border-stone-200 py-2.5 pl-9 pr-3 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500"
                  />
                </div>
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-6 pb-4 flex items-center justify-between">
        <p className="text-sm text-stone-500">{filtered.length} restaurant{filtered.length === 1 ? '' : 's'} available</p>
        <span className="inline-flex items-center gap-1 rounded-full bg-stone-100 px-3 py-1 text-xs font-medium text-stone-600"><SlidersHorizontal className="h-3.5 w-3.5" /> Smart filters</span>
      </section>
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-6 pb-16 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          >
            ...
            <div className="p-4">
              <div className="flex items-start justify-between gap-2">
                <h3 className="font-semibold text-stone-900 group-hover:text-amber-700">{r.name}</h3>
              </div>
        ...
        {filtered.length === 0 && (
          <p className="text-stone-500 col-span-full text-center py-12">No restaurants match your search.</p>
        )}
      </section>
    </div>
  )
