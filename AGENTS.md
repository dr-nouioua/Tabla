...
- **WhatsApp:** never call a real WhatsApp API directly from a route. Always go through
  `sendWhatsappMessage()` in `whatsapp.server.ts`, which is the single seam to swap in a real provider later.
